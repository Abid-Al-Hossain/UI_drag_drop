# UI Foundry Drag Drop Studio

Standalone visual Drag Drop generator built with Next.js, React, and TypeScript.

Standalone Drag Drop component studio for building native, preview-honest, React-exportable UI patterns that match the UI Foundry product line.
...
- designers building Drag Drop UI quickly
- developers who need editable React starting points
- marketplace buyers who want a self-contained Drag Drop generator
...
The editor is focused on controls that are native to Drag Drop and useful in real product work:
- draggable items, dropzone, active drag, over state, reorder preview, and disabled state
- item/card surface, handle, ghost, placeholder, gap, radius, border, shadow, and motion-safe feedback
- kanban, upload dropzone, sortable list, grid reorder, and builder variants
- keyboard alternative and accessible drag/drop guidance
- React export for drag/drop presentation

The goal is maximum useful depth without adding unrelated controls that would make the component stop reading as Drag Drop.
...
This repo is intended to publish as Abid-Al-Hossain/UI_drag_drop.
