...
export default function ItemsSection({ state, update }: Props) {
  return <SectionCard title="Items" subtitle="Items controls for native dragdrop generation.">
      <div className="space-y-4"><Slider label="Item count" value={state.itemCount} min={0} max={14} step={1} onChange={(value) => update("itemCount", value)} />
...
<Input label="Drop zone label" value={state.dropZoneLabel ?? "Selected drop zone"} onChange={(value) => update("dropZoneLabel", value)} /></div>
    </SectionCard>;
}
