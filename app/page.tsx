...
import { DEFAULT_DRAGDROP_STATE } from "./_data/DragDropPresets";
...
import { SECTIONS, type SectionId, type DragDropState, type StudioPreset } from "./types";

export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<DragDropState>(DEFAULT_DRAGDROP_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const [activePresetId, setActivePresetId] = useState<string | null>(null);
  const [downloadName] = useState("drag-drop-component");
  ...
  const update = <K extends keyof DragDropState>(key: K, value: DragDropState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_DRAGDROP_STATE, ...(preset.state as Partial<DragDropState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
...
return <AppShell contentOverflow="hidden"><PlaygroundLayout title="Drag Drop Studio" headerActions={headerActions} controls={controls} preview={output} /><ContrastGuard /></AppShell>;
}
